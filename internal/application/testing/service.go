...
// parseMarkdownCases parses the standard .md format generated by Claude
func parseMarkdownCases(content string) []*domain.TestCase {
	var cases []*domain.TestCase
	sections := strings.Split(content, "\n---\n")
	for _, section := range sections {
		tc := parseOneCase(section)
		if tc != nil && tc.Title != "" {
...
func parseOneCase(section string) *domain.TestCase {
	...
	for _, line := range lines {
		if strings.HasPrefix(line, "## TC:") {
			tc.Title = strings.TrimSpace(strings.TrimPrefix(line, "## TC:"))
		} else if strings.HasPrefix(line, "priority:") {
			p := strings.TrimSpace(strings.TrimPrefix(line, "priority:"))
			tc.Priority = domain.Priority(p)
		} else if strings.HasPrefix(line, "preconditions:") {
			tc.Preconditions = strings.TrimSpace(strings.TrimPrefix(line, "preconditions:"))
		} else if strings.HasPrefix(line, "expected_result:") {
			tc.ExpectedResult = strings.TrimSpace(strings.TrimPrefix(line, "expected_result:"))
